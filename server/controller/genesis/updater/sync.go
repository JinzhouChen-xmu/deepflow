...
func (v *GenesisSyncRpcUpdater) LoadConfig(config config.GenesisConfig) {
	v.multiNSMode = config.MultiNSMode
	v.singleVPCMode = config.SingleVPCMode
	v.vmNameField = config.VMNameField
	v.defaultVPCName = config.DefaultVPCName
	v.hostIPsRanges = common.IPsToPrefixes(v.orgID, config.HostIPs)
	v.localIPRanges = common.IPsToPrefixes(v.orgID, config.LocalIPRanges)
	v.excludeIPRanges = common.IPsToPrefixes(v.orgID, config.ExcludeIPRanges)
	...
	if vmNameField := domainConfigJson.Get("vm_name_field").MustString(); vmNameField != "" {
		v.vmNameField = vmNameField
	}
	...
	if hostIPsString := domainConfigJson.Get("host_ips").MustString(); hostIPsString != "" {
		hostIPsString = strings.ReplaceAll(hostIPsString, "，", ",")
		hostIPs := strings.Split(hostIPsString, ",")
		v.hostIPsRanges = common.IPsToPrefixes(v.orgID, hostIPs)
	}
	...
	if ignoreNICRegexString := domainConfigJson.Get("ignore_nic_regex").MustString(); ignoreNICRegexString != "" {
		ignoreNICRegex, err := regexp.Compile(ignoreNICRegexString)
		if err != nil {
			log.Errorf("compile ignore NIC regex (%s) failed: %s, use yaml config", ignoreNICRegexString, err, logger.NewORGPrefix(v.orgID))
		} else {
			v.ignoreNICRegex = ignoreNICRegex
		}
	}
